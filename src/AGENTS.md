...
=== project/api-delivery rules ===

# API Quality Gates (Project-Specific)

- For every new or changed API route, add/update a **minimal feature test** set that includes:
  - happy path
  - validation failures using **data providers**
  - at least one edge-case scenario
- Keep and update `openapi.yaml` as part of the same API change so the spec stays aligned with implementation.
