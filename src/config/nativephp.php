...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | NativePHP Queue Workers (Desktop Child Processes)
    |--------------------------------------------------------------------------
    |
    | Jump / Mobile runtime does not expose the desktop child-process API
    | endpoint (localhost:4000), so auto-starting NativePHP queue workers
    | causes fatal connection errors. We run queue workers separately via
    | Docker / server process, so keep this empty for mobile development.
    |
    */
    'queue_workers' => [],
